	       ...
	       const WindowType WType, const BWPPrefilteringType BWPType)
{
  ...
  while (BHigh < FilterEnd) {
	...
	/* Bilineare */
      case BWPBilinear:
	/* Calcola l'intervallo di finestratura */
	WLen = 1 + (int) (ceil(0.5 * (FBS + A * (1.0 - ((BHigh - FilterBegin) / (B - (BHigh - FilterBegin) * Q))))) * 2);
	break;
      }					
    }
    ...
    /* Riporta la banda */
    printf("Band: %3d, %7.1f - %7.1f Hz, FIR, ", (int) Band, (double) (BLow * SampleFreq) / 2, (double) (BHigh * SampleFreq) / 2);
    fflush(stdout);				
    ...
    //FftConvolve(FIRFilter,FilterLen,CIn,IBS,COut,CA,CB);				
    FftwConvolve(FIRFilter,FilterLen,CIn,IBS,COut);
