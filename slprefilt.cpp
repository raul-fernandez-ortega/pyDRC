...
void FastLowPassFir(DLReal * Filter,const unsigned int Order,const DLReal Freq, DLReal * Window,const unsigned int WSize)
{
  ...
  if (WSize > 0) {
    ...
    if (Order%2 == 0) {
      /* Inizializza le variabili per il calcolo della ricorsione per il filtro */
      ...
      SS1 = ((DLReal) DLSin(C)) * ((DLReal) DLCos(((DLReal) 0.5) * C)) + (SR1 * (((DLReal) 1.0) - ((DLReal) DLCos(C))));
	...
	Filter[J] = Filter[K] = (DLReal) (Window[J] * SV1 / ((I - ((DLReal) 0.5)) * ((DLReal) M_PI)));
    ...
    /* Verifica l'ordine del filtro */
    if (Order%2 == 0) {
      /* Inizializza le variabili per il calcolo della ricorsione per la finestratura */
      ...
      CS1 = (CS1 * (((DLReal) 1.0) - ((DLReal) DLCos(C1)))) - ((DLReal) DLSin(HalfOrder * C1)) * ((DLReal) DLSin(C1));
      ...
      /* S = (S * (1 - cos(cd))) - sin(cv) * sin(cd); */
      CS2 = (CS2 * (((DLReal) 1.0) - ((DLReal) DLCos(C2)))) - ((DLReal) DLSin(HalfOrder * C2)) * ((DLReal) DLSin(C2));
	...
	Window[J] = WV = (DLReal) (((DLReal) 0.42) - ((DLReal) 0.5) * CV1 + ((DLReal) 0.08) * CV2);
	...
	CV2 += CS2;
      ...
      /* S = (S * (1 - cos(cd))) - sin(cv) * sin(cd); */
      CS1 = (CS1 * (((DLReal) 1.0) - ((DLReal) DLCos(C1)))) - ((DLReal) DLSin((HalfOrder + 1) * C1)) * ((DLReal) DLSin(C1));
      ...
      /* S = (S * (1 - cos(cd))) - sin(cv) * sin(cd); */
      CS2 = (CS2 * (((DLReal) 1.0) - ((DLReal) DLCos(C2)))) - ((DLReal) DLSin((HalfOrder + 1) * C2)) * ((DLReal) DLSin(C2));
	       ...
	       const WindowType WType, const SLPPrefilteringType SLPType)
{
  ...
  if ((WType == WLeft) || (WType == WFull)) {
	...
	/* Segnala lo stato */
	printf("L - Band: %3d, %7.1f Hz, width: %6d, FIR, ", (int) Band, (double) (BCut * SampleFreq) / 2, CWL);
