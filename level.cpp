		...
		const int SampleFreq, const DLReal StartFreq, const DLReal EndFreq, const DLReal W, const int MExp)
{
  ...
  if (DLStart >= (DLReal) 1.0) {
	...
	if (DLMin < (DLReal) 0.0) {
	  ...
	  /* Verifica se il minimo locale è inferiore al livello minimo e ricalcola i fattori di compressione */
	  if (DLMin < RMSLevel)
		 ...
		 const int SampleFreq, const DLReal StartFreq, const DLReal EndFreq, const DLReal W, const int MExp)
{
  ...
  if (PLStart >= (DLReal) 1.0) {
	...
	if (PLMax < (DLReal) 0.0) {
	  ...
	  /* Verifica se il massimo locale è superiore al livello massimo e ricalcola i fattori di compressione */
	  if (PLMax > RMSLevel)
