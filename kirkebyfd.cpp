...
bool KirkebyFDInvert(const DLReal * InSig, int InSigLen,DLReal * InvFilter, int InvFilterLen, 
		     const DLReal * EffortSig, int EffortSigLen, DLReal EffortFactor, int MExp)
{
  ...
  if (EffortSig != NULL) {
    for (I = 0;I < FS;I++)
      FFTArray1[I] = std::conj<DLReal>(FFTArray1[I])/(FFTArray1[I]*std::conj<DLReal>(FFTArray1[I])+EffortFactor*(EMax-FFTArray2[I]*std::conj<DLReal>(FFTArray2[I])));
  } else {
    for (I = 0;I < FS;I++)
      FFTArray1[I] = std::conj<DLReal>(FFTArray1[I])/(FFTArray1[I]*std::conj<DLReal>(FFTArray1[I])+EffortFactor);
  }
...
bool PEISMPKirkebyFDInvert(const DLReal * MPSig, int MPSigLen, const DLReal * EPSig, int EPSigLen,
			   DLReal * InvFilter,	int InvFilterLen, DLReal EffortFactor, int MExp)
{
  ...
  for (I = 0;I < FS;I++) {
    ...
    /* Applica il fattore di effort controllando eventuali singolarità */
    if (EPAbs <= (DLReal) 0.0)
      EPAbs = (DLReal) 0.0;
    else
      if (EPAbs >= (DLReal) 1.0)
    ...
    FFTArray2[I] = std::polar<DLReal>(EPAbs,-std::arg<DLReal>(FFTArray2[I])) + std::polar<DLReal>((DLReal) 1.0 - EPAbs,(I * M_PI * EPSigLen) / FS);
  }
...
bool PEMSMPKirkebyFDInvert(const DLReal * MPSig, int MPSigLen, const DLReal * EPSig, int EPSigLen,
			   DLReal * InvFilter,	int InvFilterLen, DLReal EffortFactor, int PEStart, int PETransition, int MExp)
{
  ...
  /* Effettua la fast deconvolution selettiva della componente EP */
  for (I = 0;I < FS;I++) {
    /* Normalizza il valore assoluto */
    EPAbs = AEPAbs[I] / EMax;
    ...
    /* Calcola la trasformata della deconvoluzione selettiva */
    FFTArray2[I] = std::polar<DLReal>((DLReal) EPAbs,-std::arg<DLReal>(FFTArray2[I])) + std::polar<DLReal>(1.0 - EPAbs,(I * M_PI * EPSigLen) / FS);
  }
...
bool PETFDInvert(const DLReal * MPSig, int MPSigLen, const DLReal * EPSig, int EPSigLen,
	...
	int SampleRate, int MExp)
{
  ...
  /* Copia l'array sorgente in quello temporaneo in ordine inverso in modo da ottenere direttamente il filtro inverso */
  ...
  if (PEType == 'f') {
    ...
    for (I = 0; I < FS - (PELowerWindow + EPSigLen / 2);I++)
      EPIPERemove[I] = 0;
    SpacedBlackmanWindow(&EPIPERemove[FS-(PELowerWindow+EPSigLen/2)],PELowerWindow+EPSigLen/2,(PELowerWindow+EPSigLen/2)-2*(PELowerWindow-PEUpperWindow),WLeft);
    ...
    SLPreFilt(&EPIPERemove[FS - (PELowerWindow + EPSigLen / 2)], 2 * PELowerWindow,2 * PEUpperWindow,PEFilterLen,
	      PEBandSplit,PEWindowExponent,SampleRate,PEStartFreq,PEEndFreq, 2 * PEUpperWindow,FSharpness,EPPFOut,WLeft,SLPType);

    /* Finestratura finale segnale prefiltrato */
    SpacedBlackmanWindow(&EPPFOut[EPPFOutSize / 2 - PELowerWindow], 2 * PELowerWindow,2 * PEUpperWindow,WLeft);
...
bool MRefPETFDInvert(const DLReal * MPSig, int MPSigLen, const DLReal * EPSig, int EPSigLen,
	DLReal * InvFilter,	int InvFilterLen, char PEType, int PELowerWindow, int PEUpperWindow,
	DLReal PEStartFreq, DLReal PEEndFreq, int PEFilterLen, DLReal FSharpness, int PEBandSplit,
	...
	const SLPPrefilteringType RSLPType, const DLReal OGainFactor, int SampleRate,	int MExp)
{
  ...
  if (RPEType != 'n') {
    ...
    for (I = 0;I < FS;I++)
      /* FFTArray2[I] *= std::polar<DLReal>(((DLReal) 1.0), std::arg(EPFDRef[I])); */
	...
	EPIPERemove[I] = 0;
      SpacedBlackmanWindow(&EPIPERemove[FS-(RPELowerWindow+EPSigLen/2)],RPELowerWindow+EPSigLen/2,(RPELowerWindow+EPSigLen/2)-2*(RPELowerWindow-RPEUpperWindow),WLeft);
    } else {
      /* Calcola la dimesione effettiva del filtro */
      if (RPEFilterLen % 2 == 0)
	RPEFilterLen--;
	...
	return false;
      ...
      /* Finestratura finale segnale prefiltrato */
      SpacedBlackmanWindow(&EPPFOut[EPPFOutSize / 2 - RPELowerWindow], 2 * RPELowerWindow,2 * RPEUpperWindow,WLeft);
