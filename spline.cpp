			    ...
			    const CSplineType SPType, DLReal * SPD2YN, DLReal * UN)
{
  ...
  case SplineBDef:
    SPD2YN[0] = (DLReal) -0.5;
    UN[0] = (((DLReal) 3.0) / (XN[1] - XN[0])) * ((YN[1] - YN[0]) / (XN[1] - XN[0]) - SPD1Y1);
    break;
  ...
  for (IM1 = 0,I = 1,IP1 = 2;I < N - 1;IM1++,I++,IP1++) {
    ...
    UN[I] = (YN[IP1] - YN[I]) / (XN[IP1] - XN[I]) - (YN[I] - YN[IM1]) / (XN[I] - XN[IM1]);
    UN[I] = (((DLReal) 6.0) * UN[I] / (XN[IP1] - XN[IM1]) - S * UN[IM1]) / P;
  }
  ...
  case SplineBDef:
    Q = (DLReal) 0.5;
    U = (((DLReal) 3.0) / (XN[N - 1] - XN[N - 2])) * (SPD1YN - (YN[N - 1] - YN[N - 2]) / (XN[N - 1] - XN[N - 2]));
    break;
  ...
  SPD2YN[N - 1] = (U - Q * UN[N - 2]) / (Q * SPD2YN[N - 2] + ((DLReal) 1.0));
  for (I = N - 2,IP1 = N - 1;I >= 0;I--,IP1--)
    SPD2YN[I] = SPD2YN[I] * SPD2YN[IP1] + UN[I];
}

/* Prepara la base per la spline cubica interpolante, ponendola in SPD2YN */
bool SplinePrepare(const DLReal * XN, const DLReal * YN,
		   const int N, const DLReal SPD1Y1, const DLReal SPD1YN,
...
DLReal SplineValue(const DLReal * XN, const DLReal * YN, const int N, const DLReal * SPD2YN, const DLReal X)
{
  ...
  W = (XN[IS] - X) / DX;
  Q = (X - XN[II]) / DX;
  return W * YN[II] + Q * YN[IS] + ((W * W * W - W) * SPD2YN[II] + (Q * Q * Q - Q) * SPD2YN[IS]) * (DX * DX) / (DLReal) 6.0;
}
...
void ASplineValue(const DLReal * XN, const DLReal * YN, const int N, const DLReal * SPD2YN, const DLReal * X, const int XS, DLReal * Y)
{
  ...
  while (IXP < XS) {
    ...
    while (IXP < IFU) {
      ...
      /* Calcola il valore nel punto */
      Y[IXP] = W * YNL + Q * YNU + ((W * W * W - W) * SPD2YNL + (Q * Q * Q - Q) * SPD2YNU) * DX2 / (DLReal) 6.0;
		     ...
		     const int N, const DLReal X)
{
  ...
  /* (((-t+3)*t-3)*t+1)/6 */
  BV = (((-RX + (DLReal) 3.0) * RX - (DLReal) 3.0) * RX + (DLReal) 1.0) / (DLReal) 6.0;
  if (IL > 0)
    Y += YN[IL - 1] * BV;
  else
  ...
  /* (((3*t-6)*t)*t+4)/6 */
  BV = (((((DLReal) 3.0) * RX - (DLReal) 6.0) * RX) * RX + (DLReal) 4.0) / (DLReal) 6.0;
  Y += YN[IL] * BV;

  /* Fattore di blending per IR */
  /* (((-3*t+3)*t+3)*t+1)/6 */
  BV = (((((DLReal) -3.0) * RX + (DLReal) 3.0) * RX + (DLReal) 3.0) * RX + (DLReal) 1.0) / (DLReal) 6.0;
  Y += YN[IR] * BV;
		       ...
		       const int N, const DLReal X)
{
  ...
  /* Fattore di blending per IL - 1 */
  /* ((-t+2)*t-1)*t/2 */
  BV = (((-RX + (DLReal) 2.0) * RX - (DLReal) 1.0) * RX) / (DLReal) 2.0;
  if (IL > 0)
    Y += YN[IL - 1] * BV;
  else
    Y += (((DLReal) 2.0) * YN[0] - YN[1]) * BV;
  ...
  /* Fattore di blending per IL */
  /* (((3*t-5)*t)*t+2)/2 */
  BV = (((((DLReal) 3.0) * RX - (DLReal) 5.0) * RX) * RX + (DLReal) 2.0) / (DLReal) 2.0;
  Y += YN[IL] * BV;
  
  /* Fattore di blending per IR */
  /* ((-3*t+4)*t+1)*t/2 */
  BV = (((((DLReal) -3.0) * RX + (DLReal) 4.0) * RX + (DLReal) 1.0) * RX) / (DLReal) 2.0;
  Y += YN[IR] * BV;
  ...
  while (IXP < XS) {
    ...
    while (IXP < IFU) {
      ...
      Y[IXP] = ((YNLL * ((((-RX + (DLReal) 2.0) * RX - (DLReal) 1.0) * RX)))
		...
		+ (YNUU * (((RX - (DLReal) 1.0) * RX * RX)))) / ((DLReal) 2.0);
...
bool AB3CRMSplineValue(const DLReal * XN, const DLReal * YN,
		       const int N, const DLReal * X, const int XS, DLReal * Y)
{
...
bool AB3HSMSplineValue(const DLReal * XN, const DLReal * YN,
		       const int N, const DLReal * X, const int XS, DLReal * Y)
{
  /* Array gestione spline rimappatura */
  DLReal * SX;
		     ...
		     const CSLSplineType SplineType, const CSLAxisType AxisType)
{
  ...
  case CSLBSpline:
    RV = AB3HSMSplineValue(XNE,YNE,N,XSE,XS,Y);
    break;
